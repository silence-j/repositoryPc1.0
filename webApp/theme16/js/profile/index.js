...
ProfileApi.getProfile({shopId}).then(res => {
  if (res.data && !res.data.code) {
    ...
    // 主题默认图片
    let compDescImgWhole = res.data.data.compDescImgWhole ? res.data.data.compDescImgWhole : '/assets/theme16/images/default_profile_compay_desc.jpg'   // 简介背景图
    ...
    if (compBackgroundWhole !== '' && compBackgroundWhole !== null && compBackgroundWhole !== undefined) {
      ...
      $('#profile-topBanner img').attr('src', '/assets/theme16/images/default_profile_bg.png') // /images/banner-loading.png
    }
  } else {
    // 主题默认图片
    // $('#profile-topBanner img').attr('src', '/images/dif_descImg.png')
