...
homeApi.getAdvert({shopId, position: 'AD'}).then(res => {
  if (res.data && !res.data.code) {
    ...
    res.data.data.push({
      linkTag: -1,
      linkUrl: 'javascript:;',
      pircUrlWhole: '/assets/theme16/images/default_advert_1.png'
    })
    res.data.data.push({
      linkTag: -1,
      linkUrl: 'javascript:;',
      pircUrlWhole: '/assets/theme16/images/default_advert_2.png'
    })
    res.data.data.push({
      linkTag: -1,
      linkUrl: 'javascript:;',
      pircUrlWhole: '/assets/theme16/images/default_advert_3.png'
    })
    ...
    res.data.data.forEach((obj, i) => {
      ...
      if (i < 3) {
        ...
        if (obj.linkTag === 0) {
          advert = `<div class="${itemClass} img${i + 1} img animated"><a href="${obj.linkUrl}" target="_blank" style="background-image: url(${obj.pircUrlWhole})"><img src="${obj.pircUrlWhole}"></a></div>`
        } else if (obj.linkTag === 1) {
          advert = `<div class="${itemClass} img${i + 1} img animated"><a href="/detail?shopId=${shopId}&productId=${obj.linkUrl}" style="background-image: url(${obj.pircUrlWhole})" target="_blank"><img src="${obj.pircUrlWhole}"></a></div>`
        } else if (obj.linkTag === -1) {
          advert = `<div class="${itemClass} img${i + 1} img animated"><a href="javascript:;" style="background-image: url(${obj.pircUrlWhole})" target="_blank"><img src="${obj.pircUrlWhole}"></a></div>`
        }
