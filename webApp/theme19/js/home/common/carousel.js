...
const $carouselIndicators = $('.carousel-indicators-container.dots .carousel-indicators', $container)
const $carouselThumbIndicators = $('.carousel-indicators-container.thumbs .carousel-indicators', $container)
...
console.log('准备发起getAdvert')
homeApi.getAdvert({shopId, position: 'BANNAER'}).then((res) => {
  console.log('请求返回getAdvert')
  console.log(res)
  console.log(res.data)
  console.log('请求返回getAdvert end')
  if (res.data && !res.data.code) {
    let carouselNd = ''
    let carouselIdc = ''
    let carouselThunmbIdc = ''
    let active = 'active'
    ...
    res.data.data.forEach((obj, i) => {
      ...
      if (i === 0) {
        ...
        carouselThunmbIdc += `<li data-target="#carousel-banner" data-slide-to="${i}" class="active"><div class="img-bg" style="background-image: url(${obj.pircUrlWhole})"></div></li>`
      } else {
        ...
        carouselThunmbIdc += `<li data-target="#carousel-banner" data-slide-to="${i}"><div class="img-bg" style="background-image: url(${obj.pircUrlWhole})"></div></li>`
      }
    })
    $carouselInner.append(carouselNd)
    $carouselIndicators.html(carouselIdc)
    $carouselThumbIndicators.html(carouselThunmbIdc)
  }
})
