 ...
 * Date: 17/3/27
 */
import '../../../common/js/home/loginbyfg.js'
...
import mainTpl from '../../views/detail/indexTemplate/mainPictpl.jade'
...
let mainEl = $('#main-pic')
let com = {
  unitsMap: {'Cubic Foot': 'Feet', 'Foot': 'Feet', 'Square Foot': 'Feet', 'Box': 'es', 'Cubic Inch': 'es', 'Gross': 'es', 'Inch': 'es', 'Perch': 'es', 'Quart': 'es', 'Square Inch': 'es', 'Hertz': '', 'Kilohertz': '', 'Megahertz': ''},
  supportMap: {'1': 'Refuse', '0': 'Available'},
  dayUnitMap: {'2': 'Day', '0': 'Week', '1': 'Month'}
}
...
let detail = {
  ...
  render () {
    ...
    if (prod.quotationMethod === 0) { // 0数量
      ...
      Object.assign(info, {productNumPrices: data.productNumPrices})
    ...
    // 商品主图
    Object.assign(info, prod, extend, {iSBindPayAccount: data.iSBindPayAccount})
    ...
    if (data.properties) {
      data.properties.forEach(item => {
        attrdesc += `<li class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><label title="${item.propertyNameEn}">${item.propertyNameEn}</label><em>:</em> <span title="${item.propertyValueNameEn}">${item.propertyValueNameEn}</span></li>`
      })
    }
    attrArr.forEach(item => {
      if (item) {
        ...
        attrdesc += `<li class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><label title="${lbl}">${lbl}</label><em>:</em> <span title="${spn}">${spn}</span></li>`
      }
    })
    ...
    detail.params = {'details[0].productId': detail.pid, 'details[0].productName': prod.productName, 'details[0].productModel': prod.productModel, 'details[0].productPic': extend.productPicUrls.split(';')[0], 'details[0].productPrice': prod.priceUnit + ' ' + Utils.toFl(prod.priceMin) + '-' + Utils.toFl(prod.priceMax)}
    detail.phoneSwiper()
    // })
  },
  events () {
    ...
    mainEl.on('change', '.spec-select .ui-select', (e) => {
      ...
      for (let i = 0, len = allselect.length; i < len; i++) {
        ...
        let lbl = allselect.eq(i).prev('label').text()
    ...
    // 小图片显示
    mainEl.on('mouseover', '#img-list>a', (e) => {
      let _this = $(e.target)
  ...
  getShareUrl (type) {
    console.log(apiHost + '/buyer/product/genshareinfo?shopId=' + detail.shopId + '&productId=' + detail.pid + '&type=' + type)
  ...
  buynowValid () {
    ...
    let specVN = ''
    sel.each((i, item) => {
      specVN += $(item).prev('label').text() + ':' + $(item).val() + ';'
    })
