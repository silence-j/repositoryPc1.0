...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__verification_js__ = __webpack_require__(94);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__userApi__ = __webpack_require__(58);
/**
 * @author monkeywang
 * Date: 17/4/13
 */
...
var $ = window.$;
var $loginAccount = $('.username');
var $password = $('.password');
var $captcha = $('.captcha');
var $loginBtn = $('.login-btn');
var $forgot = $('.forgot');
var $goToLogin = $('.go-to-login');
// let $facebook = $('.facebook-login')
var $validateInput = $('.validate-input');
var $confirmNewpasswd = $('#confirm-new-passwd');
var $newPasswd = $('#new-passwd');
var plat = window.plat;
...
__WEBPACK_IMPORTED_MODULE_1__userApi__["a" /* default */].facebookOuth(url).then(function (res) {
  if (res.data.code === 0) {
    if (plat === 1) {
      window.location.href = res.data.data;
    } else {
      $('.social-login-wrap').on('click', '.facebook-login', function (e) {
...
__WEBPACK_IMPORTED_MODULE_1__userApi__["a" /* default */].ggLoginj({ callbackHtml: callbackHtml }).then(function (res) {
  if (res.data.code === 0) {
    $('.social-login-wrap').on('click', '.gg-login', function (e) {
      window.open(res.data.data, 'newwindow', 'height=400, width=600, toolbar =no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
    });
  }
});
 ...
 * redisKey
 */
__WEBPACK_IMPORTED_MODULE_1__userApi__["a" /* default */].getUserKey();
/**
 * 登录页面表单验证
 * @return {[type]} [description]
 */
function validateLogin() {
  ...
  return __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkEmail(email) && __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkPassword(passwd);
}
/**
 * 设置密码界面，填写email
 * @return {[type]} [description]
...
function validateSetpasswd1() {
  var emailPwd = $('#email').val();
  // console.log(email + 'email' + Validator.checkEmail(email))
  // console.log(passwd + 'passwd:' + Validator.checkPassword(passwd))
  return __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkEmail(emailPwd);
}
/**
 * 设置密码界面2 填写密码
 * @return {[type]} [description]
...
function validateSetpasswd2() {
  var pswd = $('#new-passwd').val();
  return __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkPasswordTwo(pswd);
}
/**
 * 注册登录按钮 的显示
 * @return {[type]} [description]
...
function validateRegister() {
  ...
  var pswdReg = $('#new-passwd').val();
  var pswdEqual = $newPasswd.val() === $confirmNewpasswd.val();
  return __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkEmail(emailReg) && __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkPasswordTwo(pswdReg) && pswdEqual;
}
...
function loginsub() {
  ...
  __WEBPACK_IMPORTED_MODULE_1__userApi__["a" /* default */].login(loginAccount, password, captcha).then(function (res) {
    console.log('res', res);
    if (res.data.code === 1) {
      ...
      console.log(5555);
      localStorage.setItem('isLogin', 1);
      var currHref = localStorage.getItem('currHref');
...
$validateInput.on('input', function (e) {
  ...
  var vtype = $thisObj.attr('vtype');
  switch (vtype) {
    case 'email':
      var ifValidEmail = __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkEmail(inputValue);
      if (!ifValidEmail) {
      ...
      break;
    case 'password':
      var ifValidPwd = __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkPassword(inputValue);
      if (!ifValidPwd) {
        $thisObj.parent().addClass('error');
      } else {
        console.log('延迟触发');
      ...
      break;
    case 'passwordreg':
      var ifValidPwdTwo = __WEBPACK_IMPORTED_MODULE_0__verification_js__["a" /* default */].checkPasswordTwo(inputValue);
      if (!ifValidPwdTwo) {
        $thisObj.parent().addClass('error');
      } else {
        $thisObj.parent().removeClass('error');
    ...
    case 'passwordConfirm':
      var ifValidPwdConfirm = $newPasswd.val() === $confirmNewpasswd.val();
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__userApi__ = __webpack_require__(58);
...
$register.on('click', function (e) {
  ...
  var loginAccount = $email.val();
  var password = $password.val();
  var captcha = $captcha.val();
  __WEBPACK_IMPORTED_MODULE_0__userApi__["a" /* default */].register(loginAccount, password, captcha).then(function (res) {
    if (res.data.code === 0) {
      console.log('注册成功!');
      sessionStorage.setItem('user', JSON.stringify(res.data.data));
      // var orderHref = Service.getUrlParams('orderHref')
 ...
 * @return {[type]}         [description]
 */
$showPasswd.on('click', function (e) {
 ...
 * @return {[type]}         [description]
 */
$onloonService.on('click', function (e) {
...
var $captcha = $('.captcha');
var $newPasswd = $('#new-passwd');
var $resetPwdBtn = $('#reset-pwd');
var $setNewPasswd = $('#setNewPasswd');
var $showPwd = $('.show-pwd');
/**
 * 点击 重置密码
 */
$resetPwdBtn.on('click', function (e) {
  ...
  var loginAccount = $loginAccount.val();
  var captcha = $captcha.val();
  __WEBPACK_IMPORTED_MODULE_0__userApi__["a" /* default */].checkName(loginAccount, captcha).then(function (res) {
    console.log(res);
    if (res.data.code === 0) {
      console.log('邮箱存在成功!');
       ...
       * @type {String}
       */
      __WEBPACK_IMPORTED_MODULE_0__userApi__["a" /* default */].sendMail(loginAccount).then(function (res2) {
        if (res.data.code === 0) {
 ...
 * @return {[type]}         [description]
 */
$setNewPasswd.on('click', function (e) {
  if (!$setNewPasswd.hasClass('able')) {
    console.log('灰色设置密码按钮点击');
    return;
  }
  var loginAccount = getParams('loginAccount');
  var code = getParams('code');
  var newPasswd = $newPasswd.val();
  __WEBPACK_IMPORTED_MODULE_0__userApi__["a" /* default */].setNewPasswd(loginAccount, newPasswd, code).then(function (res) {
    if (res.data.code === 0) {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__login__ = __webpack_require__(413);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__register__ = __webpack_require__(414);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__setPasswd_js__ = __webpack_require__(415);
/**
 * @author monkeywang
 * Date: 17/3/31
 */
