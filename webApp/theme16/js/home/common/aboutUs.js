...
homeApi.getAboutus({shopId: shopId, previewTag: 0}).then(res => {
  if (res.data && !res.data.code) {
    ...
    if (aboutImg === '' || aboutImg === null) {
      // 主题默认图片
      aboutImg = '/assets/theme16/images/default_home_shopDesc16.png'
    ...
    if (desc !== '' && desc !== null) {
      ...
      $('.discrible').append(`<p class="btn-wrapper"><a href="/profile?shopId=${shopId}" target="_blank" class="btn btn-default jump-to"><i class="glyphicon-icon glyphicon-plus"></i><span class="text">LEARN MORE</text></a><p>`)
      $('.text-loading').remove()
    }
  }
})
