...
module.exports = __webpack_require__(80)|| !__webpack_require__(7)(function(){
  var K = Math.random();
...
module.exports = {
  set: Object.setPrototypeOf || ('__proto__' in {} ? // eslint-disable-line
    function(test, buggy, set){
      try {
        set = __webpack_require__(73)(Function.call, __webpack_require__(62).f(Object.prototype, '__proto__').set, 2);
        set(test, []);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__detailApi__ = __webpack_require__(95);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__serveice__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_babel_polyfill__ = __webpack_require__(169);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_js_base64__ = __webpack_require__(170);
...
var base = __WEBPACK_IMPORTED_MODULE_4_js_base64___default.a.Base64;
var mainTpl = null;

var detail = {
  pid: __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].getUrlParams('productId'),
  shopId: __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].getUrlParams('shopId'),
  params: {},
  skuSpec: {},
  firstPic: '',
  ...
  render: function render() {
     ...
     * 详情数据
     */
    var res = JSON.parse(detail.prodDetail);
    // api.getDetail({productId: detail.pid}).then(res => {
    var info = {};
    var data = res.data;
    var prod = data.product;
    var extend = data.productExtends;
    ...
    if (prod.quotationMethod === 0) {
    ...
    } else if (prod.quotationMethod === 1) {
      // 1 sku
      var sku = {};
      sku.specMap = data.specMap;

      sku.skuprice = __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(prod.priceMin);
      if (prod.priceMin !== prod.priceMax) {
        sku.skuprice += ' ~ ' + __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(prod.priceMax);
      }

      Object.assign(info, sku);
    } else {
      // 2 fob
      var fob = data.productFobs[0]; // 商品FOb报价
      ...
      if (fob.fobMinPrice === fob.fobMaxPrice) {
        fob.fobprice = __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(fob.fobMinPrice);
      } else {
        fob.fobprice = __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(fob.fobMinPrice) + '~' + __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(fob.fobMaxPrice);
      }
    ...
    // 商品属性
    var descattr = $('#desc-attr');
    if (!extend.productAttr) {
      descattr.hide();
    }

    var attrArr = extend.productAttr.split(';');
    var attrdesc = '';
    if (data.properties) {
      data.properties.forEach(function (item) {
        attrdesc += '<li class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><label>' + item.propertyNameEn + ' <em>:</em></label><span title="' + spn + ('">' + item.propertyValueNameEn + '</span></li>');
      });
    }

    attrArr.forEach(function (item) {
      var lbl = item.split(':')[0];
      var spn = item.split(':')[1];
      if (!lbl || !spn) return;
      // attr[item.split(':')[0]] = item.split(':')[1]
      attrdesc += '<li class="col-lg-4 col-md-6 col-sm-6 col-xs-12"><label>' + lbl + ' <em>:</em></label><span title="' + spn + ('">' + spn + '</span></li>');
    });
    ...
    // 主图 轮播
    var maip = $('#carousel-mainpic');
    maip.carousel({
      pause: true,
      interval: false
    });
    maip.on('slide.bs.carousel', function (d) {
      var len = $(d.relatedTarget).prevAll('.item').length;
    ...
    detail.params = {
      ...
      'details[0].productPrice': prod.priceUnit + ' ' + __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(prod.priceMin) + '-' + __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(prod.priceMax)
    };
  ...
  events: function events() {
    // 数量加减
    mainEl.on('click', 'a.glyphicon-reduce', function (e) {
      var num = $(e.target).parents('.calu-wrap').find('input');
    ...
    mainEl.on('click', 'a.glyphicon-plus', function (e) {
      var num = $(e.target).parents('.calu-wrap').find('input');
      if (num.val()) {
        var n = Number(num.val());
        n++;
    ...
    mainEl.on('input', '.calu-wrap .ui-input', function (e) {
      var num = $(e.target);
      var numv = num.val();
      if (isNaN(numv)) {
        num.val(1);
    ...
    // 选择规格
    mainEl.on('change', '.spec-select .ui-select', function (e) {
      var allselect = mainEl.find('.spec-select').find('.select3-options');
      var specgroup = '';
      // let flag = false
      for (var i = 0, len = allselect.length; i < len; i++) {
        var selectli = allselect.eq(i).find('li.selected');
        if (!selectli.length) {
          selectli = allselect.eq(i).find('li:eq(0)');
        }
        var lbl = allselect.eq(i).parents('.select3').prev('label').attr('data-val');
        specgroup += lbl + ':' + selectli.text() + ';';
      }
      detail.skuPriceShow(false, specgroup);
    });
    // 下单
    mainEl.on('click', '#buy-now', function (e) {
      e.preventDefault();
      if (!detail.buynowValid()) {
        // 判断数量是否合理
        return;
      }
      var product = {
      ...
      var p = $('#spec-box');
      var amount = +p.find('.ui-input').val();
      // 规格对应的库存
      var allselect = mainEl.find('.spec-select').find('.ui-select');
      var specgroup = '';
      var flag = false;
      for (var i = 0, len = allselect.length; i < len; i++) {
      ...
      if (flag) {
        __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert('you should choice product', 'warning');
        return;
      }
      ...
      __WEBPACK_IMPORTED_MODULE_1__detailApi__["a" /* default */].addCart(product).then(function (res) {
        var data = res.data;
        if (!data.code) {
          __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert('add to shopping cart success!');
    ...
    // 询盘提交
    $('.desc-container').on('click', '.btn-send', function () {
      var params = {};
      var form = $('#form');
      ...
      __WEBPACK_IMPORTED_MODULE_1__detailApi__["a" /* default */].submitSupplier(params).then(function (res) {
        if (!res.data.code) {
          form[0].reset();
          form.find('.form-group').removeClass('valid');
          form.find('.form-group').removeClass('error');
          form.find('.btn-send').prop('disabled', true);
          __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert('Thanks for your feedback, we will have sales contact later');
        } else {
          __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert(res.data.message, 'info');
        }
      });
    });

    // 小图片显示
    var imgListTimeout = null;
    mainEl.on('mouseenter', '#img-list>a', function (e) {
      var _this = $(e.target);
    ...
    }).on('mouseleave', '#img-list>a', function (e) {
  ...
  getShareUrl: function getShareUrl(type) {
   ...
   * @return {[type]}           [description]
   */
  skuShow: function skuShow(specData, priceUnit, specMap) {
    for (var i = 0, len = specData.length; i < len; i++) {
      var spenum = specData[i].productSpecNums;
      var startNumPirce = [];
      if (spenum.length) {
        for (var j = 0, spenumlen = spenum.length; j < spenumlen; j++) {
          if (j < spenumlen - 1) {
            startNumPirce.push({ startNum: spenum[j].startNum + ' ~ ' + Number(spenum[j + 1].startNum - 1), price: __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(spenum[j].price) });
          } else {
            startNumPirce.push({ startNum: '≥ ' + spenum[j].startNum, price: __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(spenum[j].price) });
          }
        }
      }
  ...
  skuPriceShow: function skuPriceShow(isFirst, specMap) {
    var tpl = '';
    var specName = '';
    if (isFirst) {
      // 初始化
    ...
    // 显示的价格/起批量表格
    var specVals = detail.skuSpec[specName];
    var firstSpec = specVals.startNumPirce;
    var tit = '<li class="tit"><p>Price<em>(' + detail.skuSpec[specName].priceUnit + ')</em></p><p>Quantity</p></li>';
    var skuprice = $('#sku-price');
    var allskusprice = $('#sku-allprice');
    if (firstSpec.length) {
      firstSpec.forEach(function (item) {
      ...
      allskusprice.html(specVals.priceUnit + ' ' + __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].toFl(specVals.specPrice));
      allskusprice.show();
      skuprice.parents('.price-tab').hide();
    }
  },
  buynowValid: function buynowValid() {
    var parent = $('#spec-box').find('.spec-select');
    var sel = parent.find('.ui-select');
    var quantity = parent.find('.ui-input').val();
    var specVN = '';

    sel.each(function (i, item) {
      specVN += $(item).parents('.select3').prev('label').attr('data-val') + ':' + $(item).children().eq($('.ui-select').val()).text() + ';';
    });

    var specNum = detail.skuSpec[specVN];
    var isGo = true;
    if (specNum.invenory < quantity) {
      // 输入数量大于库存
      // Utils.alert('Please enter a lower number.', 'warning')
      __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert('The product does not have enough stock', 'warning');
      isGo = false;
    }

    if (specNum.spenum.length) {
      // 有起批价
      if (quantity < specNum.spenum[0].startNum) {
        // 小于最小起批量
        __WEBPACK_IMPORTED_MODULE_2__serveice__["a" /* default */].alert('Please enter a number larger than ' + specNum.spenum[0].startNum, 'warning');
  ...
  adjust: function adjust() {
    var div = $('#desc-img');
    var w = div[0].clientWidth;
    div.find('table').each(function (indx, item) {
      if ($(item).parents('table').length === 0) {
        var wd = $(item).css('width');
        var wdn = Number(wd.replace(/[^0-9]/ig, ''));
        if (wd.indexOf('pt') !== -1) {
          if (wdn * 4 / 3 > w) {
            $(item).css('width', w);
          }
...
module.exports = __webpack_require__(72).RegExp.escape;
...
$export($export.P + $export.F * (__webpack_require__(97) != Object || !__webpack_require__(68)(arrayJoin)), 'Array', {
...
if(!(TO_PRIMITIVE in proto))__webpack_require__(28)(proto, TO_PRIMITIVE, __webpack_require__(174));
...
$export($export.S + $export.F * !__webpack_require__(13), 'Object', {defineProperties: __webpack_require__(150)});
...
__webpack_require__(75)('Float32', 4, function(init){
...
__webpack_require__(75)('Float64', 8, function(init){
...
__webpack_require__(75)('Int16', 2, function(init){
...
__webpack_require__(75)('Int32', 4, function(init){
...
__webpack_require__(75)('Int8', 1, function(init){
...
__webpack_require__(75)('Uint16', 2, function(init){
...
__webpack_require__(75)('Uint32', 4, function(init){
...
__webpack_require__(75)('Uint8', 1, function(init){
...
__webpack_require__(13) && $export($export.P + __webpack_require__(106), 'Object', {
...
var Set                     = __webpack_require__(162)
  , from                    = __webpack_require__(138)
  , metadata                = __webpack_require__(74)
  , anObject                = __webpack_require__(4)
  , getPrototypeOf          = __webpack_require__(63)
