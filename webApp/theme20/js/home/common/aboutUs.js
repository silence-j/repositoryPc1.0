...
homeApi.getAboutus({shopId: shopId, previewTag: 0}).then(res => {
  if (res.data && !res.data.code) {
    ...
    if (aboutImg === '' || aboutImg === null) {
      // 主题默认图片
      aboutImg = '/assets/theme16/images/default_home_shopDesc20.png'
      // aboutImg = '/images/CombinedShape.png'
    }

    let apImg = `<img class="img-left about-img" src="${aboutImg}">`
    $('.ab-img-container').append(apImg)
    // $img.attr('src', res.data.data.wholeDescImgUrl)
    $('.img-hold').remove()
    $('#spacer').after(apImg)

    let spacr = document.getElementById('spacer')
    if (spacr) {
      sizeSpacer(spacr)
      window.onresize = function () {
        console.log('sizeSpacer')
        sizeSpacer(spacr)
      }
    }
    ...
    if (desc !== '' && desc !== null) {
      ...
      $('.discrible').append(`<div class="btn-wrapper"><a href="/profile?shopId=${shopId}" target="_blank" class="btn btn-default jump-to"><i class="glyphicon glyphicon-plus"></i><span class="text">LEARN MORE</text></a><div>`)
      $('.text-loading').remove()
    }
