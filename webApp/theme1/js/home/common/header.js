...
homeApi.getCompanyDetail({shopId}).then((res) => {
  if (!res.data.code) {
    if (res.data.data) {
      ...
      $logoImg.show()
      if (data.shopLogoWhole !== null && data.shopLogoWhole !== '') {
        $logoImg.removeClass('logo-default')
        $logoImg[0].style.backgroundImage = 'url(' + data.shopLogoWhole + ')'
      ...
      $tel.text(data.contactTelephone)
      ...
      //   $('#footer').hide()
      // }
      if (data.facebookUrl !== null && data.facebookUrl !== 'null' && data.facebookUrl !== '') {
        $icon.append(`<a href="${data.facebookUrl}" target="_blank" ><i class="glyphicon glyphicon-facebook"></i></a>`)
        $iconTop.append(`<a href="${data.facebookUrl}" target="_blank" class="ff"><i class="glyphicon glyphicon-facebook"></i></a>`)
        $xsList.append(`<a href="${data.facebookUrl}" target="_blank" class="social-container"><i class="glyphicon glyphicon-facebook"></i></a>`)
      } else {
      ...
      if (data.twitterUrl !== null && data.twitterUrl !== 'null' && data.twitterUrl !== '') {
        $icon.append(`<a href="${data.twitterUrl}" target="-_blank"><i class="glyphicon glyphicon-twitter"></i></a>`)
        $iconTop.append(`<a href="${data.twitterUrl}" target="_blank"><i class="glyphicon glyphicon-twitter"></i></a>`)
        $xsList.append(`<a href="${data.twitterUrl}" target="_blank" class="social-container"><i class="glyphicon glyphicon-twitter"></i></a>`)
      } else {
      ...
      if (data.linkedinUrl !== null && data.linkedinUrl !== 'null' && data.linkedinUrl !== '') {
        $icon.append(`<a href="${data.linkedinUrl}" target="_blank"><i class="glyphicon glyphicon-linkedIn"></i></a>`)
        $iconTop.append(`<a href="${data.linkedinUrl}" target="_blank"><i class="glyphicon glyphicon-linkedIn"></i></a>`)
        $xsList.append(`<a href="${data.linkedinUrl}" target="_blank" class="social-container"><i class="glyphicon glyphicon-linkedIn"></i></a>`)
      } else {
