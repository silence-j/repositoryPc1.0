...
const navarTop = $('.header .bottom').offset().top
  ...
  if (res.data && !res.data.code) {
    if (res.data.data) {
      ...
      if ($locationHd.length > 0) {
        ...
        $locationHd.attr('title', res.data.data.address)
      }
...
// Scroll
$(window).on('scroll', (e) => {
  if (!$('.header .top .header-left').is(':visible')) {
    return
  }
  console.log(navarTop)
