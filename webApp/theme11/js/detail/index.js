...
let detail = {
  ...
  render () {
    ...
    // 名称转换
    // prod.productName = prod.productName.replace(/&quot;/g, '\'').replace(/&quot2;&quot2;/g, '\"')
    // prod.productModel = prod.productModel ? prod.productModel.replace(/&quot;/g, '\'').replace(/&quot2;&quot2;/g, '\"'): ''
    // paymenthod
    ...
    if (prod.quotationMethod === 0) { // 0数量
      ...
      // let specData = data.productSpecs
      // for (let i = 0, len = specData.length; i < len; i++) {
      //   let spenum = specData[i].productSpecNums
      //   detail.skuSpec[specData[i].specName] = {
      //     invenory: specData[i].invenory,
      //     spenum: spenum
      //   }
      // }
  ...
  events () {
    ...
    mainEl.on('change', '.spec-select .ui-select', (e) => {
      ...
      for (let i = 0, len = allselect.length; i < len; i++) {
        ...
        //   break
        // }
        let lbl = allselect.eq(i).prev('label').attr('data-val')
  ...
  buynowValid () {
    ...
    let specVN = ''
    sel.each((i, item) => {
      specVN += $(item).prev('label').attr('data-val') + ':' + $(item).val() + ';'
    })
