...
let $pcSearchCntr = $('.top-search') // $('.navbar-search') $('.foot-search')
let $pcSearch = $('.hidden-search')
let $mbSearch = $('.mbSearch')
let $mbSearchCntr = $('.mbSearch-wrapper')

const $header = $('.header')
const $telHd = $('.tel', $header)
const $locationHd = $('.location', $header)
...
const $telLeftHalf = $('.left-half', $footer)
const $telRightHalf = $('.right-half', $footer)
...
homeApi.getCompanyDetail({shopId}).then((res) => {
  if (res.data && !res.data.code) {
    if (res.data.data) {
      ...
      if ($telHd.length > 0) {
        $telHd.text(res.data.data.contactTelephone)
      }
      if ($telLeftHalf.length > 0 && $telRightHalf.length > 0) {
        let telLen = res.data.data.contactTelephone.length
        let startLen = Math.floor(telLen / 2)
        $telLeftHalf.text(res.data.data.contactTelephone.substr(0, startLen))
        $telRightHalf.text(res.data.data.contactTelephone.substr(startLen))
      }
      $email.text(res.data.data.contactEmail)
      $location.text(res.data.data.address)
      if ($locationHd.length > 0) {
        $locationHd.text(res.data.data.address)
      }
      ...
      // 底部样式调整
      let maxHeight = 0
      $('.support .contact.context .contact-item', $footer).each(function (index, el) {
        let $inner = $(el).find('.contact-inner')
        if ($inner.length < 0 || !$inner.height()) {
          return
        }

        maxHeight = maxHeight > $inner.height() ? maxHeight : $inner.height()
      })
      $('.support .contact.context .contact-item .contact-inner', $footer).height(maxHeight)
    }
  }
})
...
$searchBar.on('click', (e) => {
  e.preventDefault()
  $pcSearchCntr.css('display', 'block')
  $pcSearch.removeClass('zoomOut').addClass('zoomIn')
  $pcSearch.css('display', 'inline-block')
  $searchBar.hide()
  $searchBar.siblings('.close-input').css('display', 'inline-block')
  $('.search-tip-list').hide()
})
$('.close-input', $pcSearch).on('click', (e) => {
  $pcSearch.removeClass('zoomIn').addClass('zoomOut').one('animationend', () => {
    $pcSearchCntr.css('display', 'none')
    $pcSearch.css('display', 'none')
    $searchBar.show()
    $searchBar.siblings('.close-input').hide()
  })
})
$('.search-li .close-input').on('click', (e) => {
  $pcSearch.removeClass('zoomIn').addClass('zoomOut').one('animationend', () => {
    $pcSearchCntr.css('display', 'none')
    $pcSearch.css('display', 'none')
    $searchBar.show()
...
$('.header').on('click', '.close-input', (e) => {
  ...
  let $closeInput = $(e.currentTarget)
  let $inputDown = $closeInput.closest('.input-group').find('.form-control')
  if ($inputDown.length > 0) {
    $inputDown.val('')
  }

  let $searchTipCntr = $closeInput.closest('.input-group').find('.search-tip-list')
  if ($searchTipCntr.length > 0) {
    $searchTipCntr.hide()
  }
})
...
.on('click', '.info-menu', (e) => {
  let $infoMenu = $(e.currentTarget)
  let $headerRight = $infoMenu.closest('.header-right')
  $headerRight.find('.info-menu-list').toggle()
})
...
$('.input-group input').on('keyup', function (event) {
  if (event.keyCode === 13) {
    let $parent = $(this).closest('.input-group')
    $parent.find('.search-btn').trigger('click')
  }
})
